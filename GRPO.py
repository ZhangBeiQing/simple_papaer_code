...
def maximize_grpo_objective(model, ref_model, rollout_data, tokenizer, reward_function, 
    ...
    # Shape: (batch_size*num_generations, logits_to_keep) - 每个token的对数概率

    # Compute policy ratio
    ratio = torch.exp(current_log_probs - old_log_probs)
    # Shape: (batch_size*num_generations, logits_to_keep) - 新旧策略的概率比
    ...
    # Shape: (batch_size*num_generations) - 每个生成结果的奖励值
    ...
    # Shape: (batch_size*num_generations, 1) - 每个生成结果相对于同组其他结果的优势值

    # Compute surrogate loss with clipping
    surrogate1 = ratio * advantages
    # Shape: (batch_size*num_generations, logits_to_keep) - 未剪切的替代损失

    surrogate2 = torch.clamp(ratio, 1 - epsilon, 1 + epsilon) * advantages
    # Shape: (batch_size*num_generations, logits_to_keep) - 剪切后的替代损失

    surrogate_loss = torch.min(surrogate1, surrogate2)
    # Shape: (batch_size*num_generations, logits_to_keep) - 最终的替代损失(取小值)

    # Compute KL divergence penalty
    kl_div = torch.exp(ref_log_probs - current_log_probs) - (ref_log_probs - current_log_probs) - 1
    # Shape: (batch_size*num_generations, logits_to_keep) - 每个token的KL散度惩罚

    # Combine losses
    per_token_loss = surrogate_loss - beta * kl_div
    # Shape: (batch_size*num_generations, logits_to_keep) - 每个token的总损失

    loss = -((per_token_loss * completion_mask).sum(dim=1) / completion_mask.sum(dim=1)).mean()
    # Shape: () (标量) - 最终的平均损失值，考虑了completion_mask来过滤无效token
